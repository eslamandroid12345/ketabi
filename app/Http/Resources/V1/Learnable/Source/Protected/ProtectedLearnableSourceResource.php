...
namespace App\Http\Resources\V1\Learnable\Source\Protected;
...
class ProtectedLearnableSourceResource extends JsonResource
{
    /**
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'introduction_platform' => $this->when(in_array($this->type, ['public_package', 'private_package']), $this->introduction_platform),
            'introduction_url' => $this->when(in_array($this->type, ['public_package', 'private_package']), $this->introduction_platform),
            ...
            'categories' => $this->when($this->categories?->count() != 0, ProtectedLearnableSourceResource::collection($this->categories)),
            'lectures' => $this->when($this->categorizedLectures?->count() != 0, ProtectedLearnableSourceResource::collection($this->categorizedLectures)),
            'uncategorized_lectures' => $this->when($this->uncategorizedLectures?->count() != 0, ProtectedLearnableSourceResource::collection($this->uncategorizedLectures)),
        ];
    }
}
